...
trait GraphMatchAnswersControllon extends Controller with SecureSocialConsented {
  ...
  def createGraphMatch(organizationId: OrganizationId, courseId: CourseId, quizId: QuizId, questionId: QuestionId) = ConsentedAction("TODO REMOVE ME WHEN INTELLIJ 14 CAN PARSE WITHOUT THIS") { implicit request => implicit user => implicit session =>
