...
trait TangentAnswersControllon extends Controller with SecureSocialConsented {
  ...
  def createTangent(organizationId: OrganizationId, courseId: CourseId, quizId: QuizId, questionId: QuestionId) = ConsentedAction { implicit request => implicit user => implicit session =>
