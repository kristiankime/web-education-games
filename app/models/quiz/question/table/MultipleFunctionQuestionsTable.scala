...
class MultipleFunctionQuestionsTable(tag: Tag) extends Table[MultipleFunctionQuestion](tag, "multiple_function_questions") with QuestionsTable[MultipleFunctionQuestion] {
	...
	def * = (id, ownerId, description, explanationRaw, explanationHtml, creationDate, atCreationDifficulty, order) <> (MultipleFunctionQuestion.tupled, MultipleFunctionQuestion.unapply _)
