...
object Users {
  ...
  def create(userSetting: User)(implicit session: Session) =
    Try(session.withTransaction { userTable.insert(userSetting); userSetting })

  def update(userSetting: User)(implicit session: Session) =
    Try(session.withTransaction {userTable.where(_.userId === userSetting.id).update(userSetting); userSetting })

  def apply(userId: UserId)(implicit session: Session) : Option[User] = userTable.where(_.userId === userId).firstOption
  ...
  def validName(startingName: String)(implicit session: Session) = {
    userTable.where(_.name === startingName).firstOption match {
      case None => startingName
      case Some(_) => {
        val similarNames = userTable.where(_.name like (startingName + "%")).list.map(_.name).toSet
