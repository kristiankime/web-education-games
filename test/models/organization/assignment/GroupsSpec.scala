  ...
  "enrolled" should {
    ...
    "return None if the user isn't enrolled in any group" in new WithApplication(FakeApplication(additionalConfiguration = inMemH2)) {
      DB.withSession { implicit session: Session =>
        ...
        val course = Courses.create(TestCourse(owner = admin.id))
        val section = Sections.create(TestSection(owner = admin.id, courseId = course.id))
        val assignment = Assignments.create(TestAssignment(owner = admin.id, courseId = course.id))
    ...
    "return the correct Group if the user is enrolled in groups for multiple assignments" in new WithApplication(FakeApplication(additionalConfiguration = inMemH2)) {
      DB.withSession { implicit session: Session =>
        ...
        val course = Courses.create(TestCourse(owner = admin.id))
        val section = Sections.create(TestSection(owner = admin.id, courseId = course.id))
        val assignment1 = Assignments.create(TestAssignment(owner = admin.id, courseId = course.id))
        val group1 = Groups.create(TestGroup(sectionId = section.id, assignmentId = assignment1.id))
        val assignment2 = Assignments.create(TestAssignment(owner = admin.id, courseId = course.id))
        val group2 = Groups.create(TestGroup(sectionId = section.id, assignmentId = assignment2.id))
