...
import models.quiz.answer.DerivativeAnswers.AnswersSummary
  ...
  "summary" should {
    ...
    "return answer if there is one" in new WithApplication(FakeApplication(additionalConfiguration = inMemH2)) {
      DB.withSession { implicit session: Session =>
        ...
        val question1 = DerivativeQuestions.create(TestDerivativeQuestion(owner = DBTest.newFakeUser.id))
    ...
    "return one row per question" in new WithApplication(FakeApplication(additionalConfiguration = inMemH2)) {
      DB.withSession { implicit session: Session =>
        ...
        val question1 = DerivativeQuestions.create(TestDerivativeQuestion(owner = DBTest.newFakeUser.id))
        val question2 = DerivativeQuestions.create(TestDerivativeQuestion(owner = DBTest.newFakeUser.id))
